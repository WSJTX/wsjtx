...
frequency-setting commands while transmitting, check *Allow
Tx-frequency corrections while transmitting*. This option permits
automatic Doppler, split/Fake It/XIT, and other Tx-frequency corrections
while the radio is keyed. Microwave EME Doppler corrections can be tens
of kHz, so WSJT-X does not limit these corrections by size. The option
never permits a nominal dial or band change while transmitting or tuning.
Transceivers known to permit

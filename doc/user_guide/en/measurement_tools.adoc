...
spectrum* from the *Tools* menu.  Wait for about a minute and then
click *Stop*.  A file named `refspec.dat` will appear in your log
directory and the *Ref Spec* checkbox on the *Wide Graph* will become
available.  When you check *Ref Spec*, the recorded reference spectrum
will then be used to flatten your overall effective passband. This
correction is applied to new received audio before spectrum display and
decoding, so it affects both the Wide Graph display and the data passed
to decoders. Clearing *Ref Spec* stops applying the correction to
subsequent received audio.

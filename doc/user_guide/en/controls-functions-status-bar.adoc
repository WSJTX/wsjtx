...
*Default* setting on the *Configurations* menu. In most modes, the next
label shows the number of messages decoded in the most recent receive
sequence. In Q65 it instead shows the number of receive sequences
accumulated for message averaging in the even and odd time slots. A
progress bar shows the elapsed fraction of a Tx or Rx sequence. Finally,
if the Watchdog

...
In 77-bit message formats, some operators prefer to send RR73 rather
than RRR.  RR73 is encoded as an explicit final response in these
formats.  In legacy 72-bit formats, RR73 matches the four-character
grid-locator form and is encoded as a grid locator, so generated Tx4
messages use RRR instead.
...
Similarly, in modes that support RR73, double-click the *Tx4* control
to toggle between sending `RRR` and `RR73` in that message.  The
`RR73` message should be used only if you are reasonably confident
that no repetitions will be required.  In legacy 72-bit and shorthand
modes, Tx4 uses `RRR` and a dialog explains why RR73 is unavailable.
...
mode other than *CQ: None*. If a CQ or QRZ message is selected, an eligible
direct caller decoded after *Enable Tx* is armed can replace it, including
immediately after the preceding QSO. Arming during reception applies only
to the remainder of that period; earlier decodes and callers from earlier
periods are not retained.
...
NOTE: The current-period caller behavior applies to direct calls to your
station. Answering a general CQ from another station is the separate
*Wait and Pounce* feature described below.
...
VHF contest.  In 77-bit modes, you can use RR73 in place of RRR, and
the final 73 is optional.
...
TIP: If you are using a compound callsign, it is recommended to
use a *Tx1* message and not a *Tx2* message when calling another station.
This will ensure that the station you are calling will be able to decode your
callsign in full.

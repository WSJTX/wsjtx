...
With *DTR* or *RTS* PTT, an external controller can hold off PTT and rig
Tune while _WSJT-X_ continues its transmit sequence and audio. Enable
*Accept UDP requests* on the *Reporting* tab to accept these TX Inhibit
commands. While enabled, _WSJT-X_ periodically advertises TX Inhibit
availability on the configured UDP reporting stream. Transmission resumes
when all holds clear, provided the transmit sequence is still active.

+

Configure UDP reporting on the *Reporting* tab so the controller can
receive _WSJT-X_ messages and send hold commands to the sending instance.
Multicast reporting allows several companion applications to receive the
messages. With unicast reporting, the controller must be the configured
receiver or receive the messages through a relay.

+

Each controller refreshes its hold while needed, then releases it. Holds
also expire if refreshes stop. With multiple controllers, PTT remains
off until every hold has cleared. Turning off *Accept UDP requests*
withdraws TX Inhibit availability and stops accepting commands; existing
holds remain until they expire.

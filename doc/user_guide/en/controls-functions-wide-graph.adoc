...
- Check *Ref Spec* to apply a pre-computed
<<REFERENCE_SPECTRUM,Reference Spectrum>> to new received audio before
spectrum display and decoding. Clearing the checkbox stops applying the
correction to subsequent audio. This control is available only when a
saved reference spectrum exists.
...
- Check *Baseline* to display a fitted baseline for the Echo spectrum.

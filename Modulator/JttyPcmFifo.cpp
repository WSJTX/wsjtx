...
JttyPcmFifo::JttyPcmFifo (qint64 capacitySamples)
  ...
  , m_drainReady {false}
  , m_drainReadySessionId {0}
  , m_drainReadyTotal {0}
{
}

bool JttyPcmFifo::enqueue (QVector<qint16> const& samples, qint64 sessionId)
{
  if (samples.isEmpty ()) return true;
  return enqueue (samples.constData (), samples.size (), sessionId);
}

bool JttyPcmFifo::enqueue (qint16 const * samples, qint64 count, qint64 sessionId)
{
...
void JttyPcmFifo::clear (qint64 sessionId)
{
  ...
  m_resetSessionId.store (sessionId, std::memory_order_release);
...
void JttyPcmFifo::applyPendingReset () noexcept
{
  ...
  qint64 const resetSessionId = m_resetSessionId.load (std::memory_order_acquire);
  ...
  m_sessionId.store (resetSessionId, std::memory_order_release);
  m_drainSignaled.store (false, std::memory_order_release);
  m_drainReady.store (false, std::memory_order_release);
  m_drainReadySessionId.store (resetSessionId, std::memory_order_release);
...
JttyPcmFifo::DrainState JttyPcmFifo::takeDrainReady () noexcept
{
  ...
  return DrainState {
    true,
    m_drainReadySessionId.load (std::memory_order_acquire),
...
qint64 JttyPcmFifo::sessionId () const noexcept
{
...
void JttyPcmFifo::updateDrainState (qint64 drainGuard) noexcept
{
  ...
  if (m_drainSignaled.compare_exchange_strong (expected, true, std::memory_order_acq_rel))
    {
      m_drainReadySessionId.store (m_sessionId.load (std::memory_order_acquire), std::memory_order_release);

...
int main(int argc, char *argv[]) {
  ...
  MMTTYIF::logMessage(QString("SENT (%1)").arg(targetName), MSG_MMTTY,
                      TXM_THREAD, static_cast<LPARAM>(threadId));
  ::PostMessageA(targetHwnd, MSG_MMTTY, TXM_THREAD,
                 static_cast<LPARAM>(threadId));

  MMTTYIF::logMessage(QString("SENT (%1)").arg(targetName), MSG_MMTTY,
                      TXM_HANDLE, reinterpret_cast<LPARAM>(hwnd));
  ::PostMessageA(targetHwnd, MSG_MMTTY, TXM_HANDLE,
                 reinterpret_cast<LPARAM>(hwnd));

  MMTTYIF::logMessage(QString("SENT (%1)").arg(targetName), MSG_MMTTY,
                      TXM_START, 0x00000000);

...
end module dopspread_mod

program mapsim

! Generate simulated data for testing of MAP65
  use iso_c_binding
  use gen_q65_cwave_mod
  use four2a_mod
  use cgen65_mod
  use noisegen_mod
  use dopspread_mod
  ...
  complex(real32) z,zx,zy
  real(real32) :: dt0, fa, fb, snrdb, rmsdb, rad, fac, pol, a, b, sig, snrdbx
  real(real32) :: rms
  real(real64) :: fcenter, fsample, twopi, phi, dphi, xphi
  real(real64) :: f, dt, samfac, fdop

  logical bq65
  character(len=24) msg0  !w3sz msg0 was size 22
  character(len=24)message
  character(len=24)msgsent
  character(len=8)arg
  character(len=11)fname
  character(len=2)mode
  character(len=24) msg_list(60) ! was *16
  integer(int16) hhmm
  ...
  call get_command_argument(11,arg) ! added by w3sz
...
!        i0=fsample*(3.5d0+0.05d0*(isig-1))
        ...
        do i=1,nwave
           ...
           xphi=phi
           z = sig * cwave(i) * cmplx( real(cos(xphi),kind=real32), &
              real(-sin(xphi),kind=real32), kind=real32 )
           zx=a*z
           zy=b*z
           j=i+i0

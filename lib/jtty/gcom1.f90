...
parameter(NRING=30*12000) !Ring buffer at 12000 samples/sec
parameter(NMAX=NRING)
real snrdb
real txsnrdb
...
common/gcom1/snrdb,txsnrdb,ndevin,ndevout,iwrite,itx,ngo,nTransmitting,nTxOK,  &
     nport,ntxed,tx_once,y1(NRING),y2(NRING),iwave(NMAX),ltx(5),lrx(5),        &
